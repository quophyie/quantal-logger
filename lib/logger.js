...
let Logger = (_dec = Wove({ bar: 42 }), _dec(_class = class Logger extends EventEmitter {
  constructor (options) {
    ...
    if (curatedOpts.useBunyanFormat) {
      ...
      curatedOpts.stream = formatOut
    }

    curatedOpts.name = curatedOpts.name || packageJson.name
    require('lugg').init(_.omit(curatedOpts, ['useBunyanFormat', 'useUuid4AsTraceId']))
        ...
        return _mdc.runAndReturn(() => {
          if (this._mainMethodCalled !== true) {
            ...
            if (this._mainMethod instanceof Function) {
              ...
              _logger.warn('The \'main\' method (entry point) of the application using this logger has not been specified in the logger configuration. ' + 'If you are using the express middleware, you can ignore this warning, otherwise, please supply ' + 'your application\'s main method (entry point) if you want to use the continuation local storage i.e. MDC')
            }
          }
        })
      })
